...
int main(int argc, char** argv)
{
    ...
    if (file) {
      ...
      while (i < 4 && std::getline(file, book)) {
        if (book[0] != '#') {
    ...
    while (std::getline(file, tempBook)) {
      if (tempBook[0] != '#') {
        library += sdds::Book(tempBook);
      }
    }
  ...
  for (auto i = 0u; i < 7; ++i) {
    ...
    if (file) {
      ...
      while (std::getline(file, movie) && index < 5) {
        if (movie[0] != '#') {
          movies[index] = sdds::Movie(movie);
          index++;
        }
      }
    } else {
